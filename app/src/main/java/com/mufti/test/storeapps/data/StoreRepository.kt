...
import com.mufti.test.storeapps.data.remote.mapper.ProductMapper.toProduct
...
class StoreRepository private constructor(
    ...
    fun getDetailProduct(idProduct: Int): LiveData<Product> = liveData {
        emit(storeDao.getProductById(idProduct).toProduct())
    }

    suspend fun getCartQuantity(idProduct: Int): Int = storeDao.getCartQuantity(idProduct) ?: 0
    ...
    suspend fun updateCartQuantityByProductId(productId: Int, quantity: Int) =
        storeDao.updateCartQuantityByProductId(productId, quantity)
